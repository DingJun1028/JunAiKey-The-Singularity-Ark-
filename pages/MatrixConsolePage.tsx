...
import { formatMarkdown } from '../components/NoteCard';
...
const StreamFinishedIcon: React.FC<{ className?: string }> = ({ className }) => (
    <svg className={`inline-block ml-2 flex-shrink-0 ${className}`} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
const MatrixConsolePage: React.FC = () => {
  ...
  const MessageBubble: React.FC<{ message: ChatMessage; isStreaming: boolean; isStreamFinished: boolean; }> = ({ message, isStreaming, isStreamFinished }) => {
    ...
    const bubbleClasses = isUser 
        ? 'bg-matrix-bg self-end border-matrix-cyan' 
        : 'bg-matrix-bg-2 self-start border-matrix-green';
    ...
    return (
        <div className={`w-fit max-w-2xl rounded-lg p-4 border ${bubbleClasses}`}>
             ...
             <div className="flex items-end">
                ...
                />
                {isStreamFinished && <StreamFinishedIcon className="text-matrix-green/50" />}
             </div>
        </div>
    );
  };
